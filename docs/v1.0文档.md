...
# 登录数据库创建业务库
mysql -uroot -p
CREATE DATABASE RoomMind DEFAULT CHARACTER SET utf8mb4;
```
...
spring:
  datasource:
    url: jdbc:mysql://127.0.0.1:3306/RoomMind?useUnicode=true&characterEncoding=utf8mb4&serverTimezone=Asia/Shanghai
    username: root
    password: 数据库密码
  servlet:
    multipart:
  ...
  web:
    upload-path: /opt/RoomMind/upload

app:
  upload:
    path: /opt/RoomMind/uploads/
    url-prefix: /uploads/
```
...
```bash
mkdir -p /opt/RoomMind
mkdir -p /opt/RoomMind/uploads

# 授权上传文件夹读写
chmod 755 /opt/RoomMind/uploads
```

2. **使用FTP工具上传**：将 `web-0.0.1-SNAPSHOT.jar` 上传到 `/opt/RoomMind`
...
```bash
mkdir -p /var/www/RoomMind
chown nginx:nginx /var/www/RoomMind
chmod 755 /var/www/RoomMind
```

2. **清空旧前端文件**

```bash
rm -rf /var/www/RoomMind/*
```
...
```bash
vim /etc/nginx/conf.d/RoomMind.conf
```
...
```bash
vim /etc/systemd/system/RoomMind.service
```
...
```ini
[Unit]
Description=RoomMind自习室预约后端服务
After=network.target mysqld.service

[Service]
Type=simple
User=root
WorkingDirectory=/opt/RoomMind
...
# 2. 设置开机自启
systemctl enable RoomMind

# 3. 立即启动后端服务
systemctl start RoomMind

# 4. 查看运行状态
systemctl status RoomMind

# 5. 实时查看后端运行日志
journalctl -u RoomMind -f

# 6. 重启后端（更新jar包后执行）
systemctl restart RoomMind

# 7. 关闭开机自启
systemctl disable RoomMind

# 8. 停止当前运行的后端
systemctl stop RoomMind
```
...
```bash
# 使用SCP上传（推荐）
scp target/web-0.0.1-SNAPSHOT.jar root@服务器IP:/opt/RoomMind/
...
# 退出SSH，上传新dist内容
exit
scp -r dist/* root@服务器IP:/var/www/RoomMind/
```
...
# 上传旧版本jar包覆盖
scp backup/web-0.0.1-SNAPSHOT.jar root@服务器IP:/opt/RoomMind/
...
# 上传旧版本dist内容
scp -r backup/dist/* root@服务器IP:/var/www/RoomMind/
```
...
| :--- | :--- |
| 数据库连接 | 确认数据库密码、端口正确 |
| 文件上传目录 | 确认 `/opt/RoomMind/uploads` 权限正确 |
| Nginx配置 | 确认反向代理路径正确 |
| SSL证书 | 确认证书文件存在且未过期 |
| 防火墙端口 | 确认7245端口已放行 |
