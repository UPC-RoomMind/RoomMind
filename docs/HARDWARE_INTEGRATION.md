...
RoomMind 自习室预约系统目前采用纯软件方式进行预约管理，用户需要手动在手机或电脑上进行签到签退操作。为提升用户体验和管理效率，计划引入硬件设备实现自动化签到签退，实现真正的"软硬结合"�?
### 1.2 目标
...
| 原则 | 说明 |
| :--- | :--- |
| **分层解�?* | 硬件层、网关层、业务层相互独立，降低耦合 |
| **接口标准�?* | API接口与硬件类型无关，支持多种验证方式 |
| **渐进式开�?* | 先开发软件模拟版本，硬件到位后快速对�?|
| **可扩展�?* | 预留扩展接口，支持未来接入更多设备类�?|
...
```
┌─────────────────────────────────────────────────────────────────────�?�?                         硬件�?                                    �?�? ┌──────────�? ┌──────────�? ┌──────────�? ┌──────────────────�?   �?�? �? RFID读卡�?�? �? 二维码扫码枪 �? �? 人脸识别终端 �? �? 门禁控制�?  �?   �?�? └────┬─────�? └────┬─────�? └────┬─────�? └────────┬─────────�?   �?└───────┼─────────────┼─────────────┼──────────────────┼──────────────�?        �?            �?            �?                 �?        �?            �?            �?                 �?┌─────────────────────────────────────────────────────────────────────�?�?                         网关�?                                    �?�? ┌──────────────────────────────────────────────────────────────�?  �?�? �?                     IoT网关 / 串口服务�?                     �?  �?�? �? 协议转换：TCP/IP <-> RS485/RS232  |  数据格式统一          �?  �?�? └──────────────────────────────────────────────────────────────�?  �?└─────────────────────────────────────────────────────────────────────�?        �?        �?┌─────────────────────────────────────────────────────────────────────�?�?                         服务�?                                    �?�? ┌────────────�? ┌─────────────�? ┌──────────────────────────�?   �?�? �?签到签退API �? �?预约状态同�?�? �?门禁控制指令（可选）      �?   �?�? └──────┬─────�? └──────┬──────�? └────────────┬─────────────�?   �?�?        �?              �?                     �?                  �?�?        �?              �?                     �?                  �?�? ┌──────────────────────────────────────────────────────────────�?  �?�? �?                  RoomMind 后端服务                        �?  �?�? �? 用户认证 | 预约验证 | 状态更�?| 积分计算 | 日志记录         �?  �?�? └──────────────────────────────────────────────────────────────�?  �?└─────────────────────────────────────────────────────────────────────�?```

### 2.2 架构说明

| 层级 | 职责 | 是否需要硬�?|
| :--- | :--- | :--- |
| **硬件�?* | 采集用户身份信息（卡号、二维码、人脸） | �?|
| **网关�?* | 协议转换、数据格式统一、设备管�?| ❌（可软件模拟） |
| **服务�?* | 业务逻辑处理、数据库操作、API接口 | �?|
...
| 方案 | 设备 | 优势 | 劣势 | 成本 |
| :--- | :--- | :--- | :--- | :--- |
| **RFID�?* | RFID读卡�?+ IC�?| 校园卡复用，无需额外卡片 | 需要携带卡�?| 低（800-1500元） |
| **二维�?* | 扫码�?| 用户出示手机即可，无需额外硬件 | 需要用户主动出�?| 中（300-800元） |
| **人脸识别** | 人脸终端 | 无接触，安全性高 | 设备成本�?| 高（1000-3000元） |

### 3.2 推荐方案：RFID�?
```
设备清单�?├── RFID读卡器：4台（每自习室1台）
├── 门禁控制器：4�?├── IC卡：按需采购（可复用校园卡）
...
ADD COLUMN IsSignedIn TINYINT(1) DEFAULT 0 COMMENT '是否已签�?,
...
CREATE TABLE device (
    ...
    DeviceType INT NOT NULL COMMENT '设备类型(1=RFID,2=扫码�?3=人脸终端)',
    RoomId INT COMMENT '所属自习室ID',
    IpAddress VARCHAR(50) COMMENT 'IP地址',
    Status TINYINT(1) DEFAULT 1 COMMENT '状�?0=离线,1=在线)',
    LastHeartbeat DATETIME COMMENT '最后心跳时�?,
...
CREATE TABLE sign_log (
    ...
    VerifyMethod VARCHAR(50) COMMENT '验证方式(RFID/二维�?人脸)',
    CardNo VARCHAR(50) COMMENT '刷卡卡号',
    QrCodeContent VARCHAR(200) COMMENT '二维码内�?,
...
| CardNo | String | �?| RFID卡号（RFID方式�?|
| QrCode | String | �?| 二维码内容（二维码方式） |
| FaceToken | String | �?| 人脸比对Token（人脸方式） |
...
| :--- | :--- | :--- | :--- |
| DeviceNo | String | �?| 设备编号 |
| CardNo | String | �?| RFID卡号 |
| QrCode | String | �?| 二维码内�?|
| FaceToken | String | �?| 人脸比对Token |
...
用户刷卡/扫码/人脸识别
       ...
       �?       �?  查询用户是否有有效预�?       �?       ├── 无有效预�?──�?返回"无预约，禁止进入"
       �?       �?  验证预约状�?必须是已预约未签�?
       �?       ├── 状态不�?──�?返回"预约状态异�?
       �?       �?  检查当前时间是否在预约时间段内
       �?       ├── 过早(提前>30分钟) ──�?返回"预约时间未到"
       �?       ├── 迟到(超过预约时间>15分钟) ──�?返回"已超时，预约已取�?
...
用户刷卡/扫码/人脸识别
       �?       �?  获取用户标识
       �?       �?  查询用户是否有已签到未签退的记�?       �?       ├── 无签到记�?──�?返回"您当前没有在自习�?
...
1. **API接口标准�?*：签到签退API不依赖具体硬件类�?2. **数据格式统一**：所有硬件数据都转换为统一的JSON格式
3. **设备抽象�?*：设备逻辑封装在独立服务中，可替换
...
**示例**�?
```bash
# 模拟签到（RFID方式�?curl -X POST http://localhost:7245/Sign/SignIn \
...
# 模拟签退（二维码方式�?curl -X POST http://localhost:7245/Sign/SignOut \
  -H "Content-Type: application/json" \
  -d '{
    "DeviceNo": "D002",
...
<template>
  <div class="sign-simulation">
    ...
    <el-select v-model="deviceNo" placeholder="选择设备">
      <el-option label="D001-第一自习�? value="D001"></el-option>
      <el-option label="D002-第二自习�? value="D002"></el-option>
      <el-option label="D003-第三自习�? value="D003"></el-option>
      <el-option label="D004-第四自习�? value="D004"></el-option>
    </el-select>
    
    <el-select v-model="verifyMethod" placeholder="验证方式">
      <el-option label="RFID刷卡" value="rfid"></el-option>
      <el-option label="二维码扫�? value="qr"></el-option>
    </el-select>
...
**适用场景**�?- 可视化测�?- 非开发人员测�?- 用户体验验证

### 7.3 开发流程建�?
| 阶段 | 是否需要硬�?| 完成内容 | 时间预估 |
| :--- | :--- | :--- | :--- |
| 第一阶段 | �?| 数据库设计、API接口开发、业务逻辑实现 | 1-2�?|
| 第二阶段 | �?| 单元测试、前端模拟页面、自动化测试脚本 | 1�?|
| 第三阶段 | �?| 异常场景测试（无预约、超时、重复签到等�?| 1�?|
| 第四阶段 | �?| 硬件安装、协议适配、联调测�?| 1�?|
| 第五阶段 | �?| 优化调整、正式上�?| 1�?|
...
| 场景 | 测试步骤 | 预期结果 |
| :--- | :--- | :--- |
| 准时签到 | 预约时间�?0分钟内签�?| 签到成功，允许进�?|
| 准时签退 | 预约时间内签退 | 签退成功，计算积�?|
| 提前签到 | 预约时间�?0分钟以上签到 | 返回"预约时间未到" |

#### 异常场景

| 场景 | 测试步骤 | 预期结果 |
| :--- | :--- | :--- |
| 无预约签�?| 没有预约记录时签�?| 返回"无预约，禁止进入" |
| 重复签到 | 已签到后再次签到 | 返回"您已签到" |
| 超时未签�?| 超过预约时间15分钟未签�?| 返回"已超时，预约已取�? |
| 无签到签退 | 没有签到记录时签退 | 返回"您当前没有在自习�? |
...
public class SignService {
    ...
    public ResultModel<SignResultDto> signIn(SignRequestDto request) {
        ...
        if (!validateAppointment(record)) {
            return ResultModel.error("预约状态异�?);
        }
...
| 服务 | 推荐配置 | 说明 |
| :--- | :--- | :--- |
| 主应用服务器 | 2�?G | RoomMind后端 |
| IoT网关服务�?| 1�?G | 处理硬件数据转发 |
| 数据�?| 2�?G | MySQL |
...
1. **分层解�?*是实现无硬件开发的核心，业务层与硬件层完全独立
2. **API接口标准�?*使得硬件更换不影响业务逻辑
3. **渐进式开�?*降低了项目风险，先完成软件模拟版�?4. **三种模拟测试方法**覆盖了不同的测试场景

### 11.2 实施建议

1. **立即开始开�?*：不需要等待硬件，可以先完成业务逻辑和API接口
2. **优先实现模拟页面**：方便测试和演示
3. **硬件采购并行**：在开发的同时采购硬件，缩短项目周�?4. **接口文档先行**：先设计好API接口文档，硬件对接时只需适配协议

### 11.3 预期效果

完成后将实现�?- 用户刷卡即可自动签到/签退
- 门禁系统与预约系统联�?- 准确的使用时长统�?- 智能积分计算
